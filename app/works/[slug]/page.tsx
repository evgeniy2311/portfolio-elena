...
export default async function WorkCasePage({ params, searchParams }: PageProps) {
  ...
  return (
    <>
      ...
      <div className="case-header shell" aria-label="Навигация кейса">
      ...
      <main id="main" tabIndex={-1}>
        <section className="case-hero shell">
          ...
          <div className="case-cover"><SlideVisual kind={item.finalSlides[0].kind} src={item.finalSlides[0].src} alt={item.finalSlides[0].alt} preload demoLabel={item.title} demo={isDemo} /></div>
      ...
      <SiteFooter>
        <div className="shell case-next-grid"><div><p>Следующий кейс · {String(adjacent.next.order).padStart(2, "0")}</p><h2 className={adjacent.next.title.length > 8 ? "long-title" : undefined}>{adjacent.next.title}</h2><p>{adjacent.next.product} · {adjacent.next.finalSlides.length} слайдов</p><Link className="button light-button" href={`/works/${adjacent.next.slug}${contextQuery}`}>Открыть следующий кейс <ArrowIcon /></Link></div><CaseStack item={adjacent.next} compact /></div>
        <div className="shell case-bottom-nav"><Link href={returnHref}><ArrowIcon direction="left" /> Все работы</Link><span>Контакт ниже</span></div>
      </SiteFooter>
    </>
  );
}
